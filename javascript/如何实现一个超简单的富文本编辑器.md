...
这就是一个富文本编辑器的全部内容了，谢谢大家.....，说笑的，实际上富文本编辑器一般还有一下的事情要处理：

* UI的部分，不可能让用户每个命令都要自己敲下代码吧，总得有些按钮什么的
* 处理IE上的兼容问题，特别是对Selection/Range上的处理
* 对一些常用的当浏览器本身没有提供的功能，要进行补全处理
* 处理图片的上传，因为一般的富文本编辑器都需要加入图片，当要在内容中插入图片，一般需要先把图片上传到服务器
...
js部分主要由两个类实现，一个是Selection类，用于第Selection的兼容处理
```javascript
    class Selection {
        constructor(window) {
            this.window = window;
...
```javascript
    ...
    toolbar.addEventListener('click', (event) => {
        ...
        // 创建一个选区
        let selection = new Selection(editor.window);
        // 并保存当前的状态
        selection.save();
